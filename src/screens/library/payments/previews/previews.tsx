import React, { useCallback, useEffect, useMemo, useState } from 'react';
import { Pressable, useWindowDimensions } from 'react-native';
import { router } from 'expo-router';
import { FlashList } from '@shopify/flash-list';

import db from '@db';
import { asc, like } from 'drizzle-orm';
import { tendersTable } from '@db/schema';

import PaymentItem from './preview-item';
import { GridItem, SectionHeader, SectionLetter, SectionRule, EmptyText } from './previews.styles';

import type { TenderT } from '@models';
import type { Props } from './previews.d';
import type { ListRenderItemInfo } from '@shopify/flash-list';

const COLUMNS = 2;
const GAP = 8;
const PADDING = 20;

type HeaderItem = { type: 'header'; letter: string };
type PaymentItem = { type: 'item'; payment: TenderT };
type ListItem = HeaderItem | PaymentItem;

const getItemType = (item: ListItem) => item.type;

const overrideItemLayout = (layout: { span?: number }, item: ListItem) => {
	if (item.type === 'header') {
		layout.span = COLUMNS;
	}
};

const keyExtractor = (item: ListItem) => (item.type === 'header' ? `header-${item.letter}` : item.payment.id);

const contentContainerStyle = { paddingHorizontal: PADDING };

const Previews = ({ search }: Props) => {
	const { width: screenWidth } = useWindowDimensions();
	const itemWidth = (screenWidth - PADDING * 2 - GAP * (COLUMNS - 1)) / COLUMNS;
	const [payments, setPayments] = useState<TenderT[]>([]);

	useEffect(() => {
		let cancelled = false;

		(async () => {
			const result = await db
				.select()
				.from(tendersTable)
				.where(like(tendersTable.title, `%${search.trim()}%`))
				.orderBy(asc(tendersTable.title));

			if (!cancelled) setPayments(result);
		})();

		return () => {
			cancelled = true;
		};
	}, [search]);

	const listData = useMemo(() => {
		const items: ListItem[] = [];

		payments.forEach((payment, index) => {
			const letter = payment.title.charAt(0).toUpperCase();
			const prev = index > 0 ? payments[index - 1].title.charAt(0).toUpperCase() : '';

			if (letter !== prev) {
				items.push({ type: 'header', letter });
			}

			items.push({ type: 'item', payment });
		});

		return items;
	}, [payments]);

	const handlePress = useCallback((id: string) => {
		requestAnimationFrame(() => {
			router.push({ pathname: '/(tabs)/library/[id]', params: { id, type: 'payment' } });
		});
	}, []);

	const renderItem = useCallback(
		({ item }: ListRenderItemInfo<ListItem>) => {
			if (item.type === 'header') {
				return (
					<SectionHeader>
						<SectionLetter>{item.letter}</SectionLetter>
						<SectionRule />
					</SectionHeader>
				);
			}

			return (
				<GridItem $width={itemWidth}>
					<Pressable onPress={() => handlePress(item.payment.id)}>
						<PaymentItem {...item.payment} />
					</Pressable>
				</GridItem>
			);
		},
		[itemWidth, handlePress]
	);

	if (!payments.length) {
		return <EmptyText>No payment methods</EmptyText>;
	}

	return (
		<FlashList
			data={listData}
			numColumns={COLUMNS}
			showsVerticalScrollIndicator={false}
			contentContainerStyle={contentContainerStyle}
			getItemType={getItemType}
			overrideItemLayout={overrideItemLayout}
			renderItem={renderItem}
			keyExtractor={keyExtractor}
		/>
	);
};

export default Previews;
