...
import { H2 } from '@ui';
import { FilterIcon, CalendarIcon } from '@ui/icons';
import { Host, Button, Switch, ContextMenu, Divider } from '@expo/ui/swift-ui';
import Root, { TitlePress, FilterBtn, GlassItem, GlassWrapper } from './lenses.styles';
...
const Lenses = () => {
	...
	return (
		<Root>
									...
									<ContextMenu.Items>
										{entries.tenders.map((tender) => (
											<Switch
												...
												label={tender.title}
												value={lensesStore.filters.some(
													(filter) => filter.type === 'tender' && filter.value === tender.id
												)}
												onValueChange={(value) => {
													if (value) {
			...
			{/*
			 * @TODO: Wait until expo-ui will support glassEffectUnion
			<Host matchContents>
				<GlassEffectContainer>
					<HStack spacing={12}>
						<SwiftButton systemImage="calendar" variant="glass" controlSize="large" />
						<SwiftButton systemImage="camera.filters" variant="glass" controlSize="large" />
					</HStack>
				</GlassEffectContainer>
			</Host>
			*/}
